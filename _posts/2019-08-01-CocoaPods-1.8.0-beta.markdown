...
CDN support was first introduced in the [1.7 release](https://blog.cocoapods.org/CocoaPods-1.7.0-beta/) and aims to speed up initial setup and dependency analysis dramatically. With 1.8, CocoaPods no longer requires cloning the now huge [master specs repo](https://github.com/CocoaPods/Specs) in order to function.
...
For more information regarding CDN, please read our previous blog post [here](https://blog.cocoapods.org/CocoaPods-1.7.2/)!

### `info_plist` Podspec DSL

CocoaPods automatically generates `Info.plist` files for Pods and Test Specs when appropriate, such as when a Podfile requires dynamic frameworks by including `use_frameworks!`.

Podspecs now support modifying the contents of their generated `Info.plist` files. While we anticipate this will be most often used to modify the bundle identifier of a framework, any key-value pair can be included. The values specified will overwrite any default values that are included by CocoaPods.

To use it, assign a Hash value to the `info_plist` attribute on a Podspec:
...
With App Specs introduced in [1.7.0](https://blog.cocoapods.org/CocoaPods-1.7.0-beta/), Podspecs can be used to describe an Application. This new `info_plist` DSL enhances the functionality of App Specs even further by allowing Podspecs to customize the App's `Info.plist`, which contains important settings such as the bundle identifier, iOS security and privacy settings, device orientation support, and more.
...
It's important to note that the `info_plist` attribute will have no effect in situations where an `Info.plist` file will not be generated, such as when Pods are integrated as static libraries. If your library requires data that's included in the `Info.plist` to always be present, we recommend including a `.plist` as a resource instead.
