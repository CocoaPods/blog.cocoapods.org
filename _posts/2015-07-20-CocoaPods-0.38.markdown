...
This is the first update since WWDC and it brings support for Apple's latest releases including Xcode 7 and watchOS.
...
Podfile allow you to define `pre_install` and `post_install` hooks. 
...
One more advanced attribute in Podspecs is - _or better was_ - the attribute `xcconfig`.
...
So Pods also can't accidentally overwrite or conflict with build settings of the user target.
...
[Changelog](https://github.com/CocoaPods/CocoaPods/blob/0.38.0/CHANGELOG.md)!
